...
# Function to get the correct password based on the server's username
get_password() {
  local server="$1"
  local user="${server%@*}"
...
execute_in_parallel() {
  ...
  for server in "${servers[@]}"; do
    # Determine the correct password for this server
    local pass=$(get_password "$server")
    exe_docker_build "$pass" "$server" &
...
read -p "Enter the indexes of the servers to target (space-separated): " -a selected_indexes
...
# Execute Docker build commands in parallel on selected servers
if [ ${#selected_servers[@]} -gt 0 ]; then
  echo "Executing Docker build on selected servers in parallel..."
