...
exe_docker_compose_down() {
  ...
  echo "Executing docker-compose down on $server with remote path $remote_dir..."

  # Execute the command and print the output
  sshpass -p "$password" ssh -o StrictHostKeyChecking=no "$server" "cd $remote_dir && docker-compose down" 2>&1 | while IFS= read -r line; do
    echo "[$server]: $line"
  done
}
...
execute_in_parallel() {
  ...
  for server in "${servers[@]}"; do
    exe_docker_compose_down "$password" "$server" &
