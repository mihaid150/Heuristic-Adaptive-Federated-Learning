...
exe_docker_image_prune() {
  ...
  echo "Doing docker image prune on $server..."

  # Execute the Docker prune command and print the output
  sshpass -p "$password" ssh -o StrictHostKeyChecking=no "$server" \
    "cd $remote_dir && yes y | docker image prune" 2>&1 | while IFS= read -r line; do
    echo "[$server]: $line"
  done
}
...
execute_in_parallel() {
  ...
  for server in "${servers[@]}"; do
    exe_docker_image_prune "$password" "$server" &
